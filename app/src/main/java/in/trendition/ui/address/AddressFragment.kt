...
import `in`.trendition.databinding.FragmentAddressBinding
...
class AddressFragment : Fragment(), AddressAdapter.AddressAdapterListener {
    ...
    private val onBackPressedCallback by lazy {
        object : OnBackPressedCallback(false) {
            override fun handleOnBackPressed() {
                binding.isAddressListEmpty = false
                if (isAdded)
                    (requireActivity() as MainActivity).toolbar?.menu?.findItem(R.id.add_address)?.isVisible =
                        true
                isEnabled = false
            }
        }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.run {
            ...
            (requireActivity() as MainActivity).toolbar?.menu?.findItem(R.id.add_address)
