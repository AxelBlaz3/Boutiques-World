...
class ProfileRepository @Inject constructor(
    ...
    fun getBoutiqueResponsesMutableLiveData(): MutableLiveData<ArrayList<BoutiqueResponse>> =
        boutiqueResponses
            ...
            if (response.isSuccessful) {
                ...
                responseBody?.apply {
                    this.retailer.let {
                        ...
                        insertRetailer(it)
                        this@ProfileRepository.retailer.postValue(it)
                    }
                }
            }
        } catch (e: Exception) {
    ...
    suspend fun updateBoutiqueRequests(businessId: Int, forceRefresh: Boolean): Boolean =
        withContext(Dispatchers.IO) {
            try {
                if (!forceRefresh) {
                ...
                val response = boutiqueService.getBoutiqueRequests(businessId).execute()
                if (response.isSuccessful) {
                    response.body()?.let {
    ...
    suspend fun updateBoutiqueResponses(businessId: Int, forceRefresh: Boolean): Boolean =
        withContext(Dispatchers.IO) {
            try {
                if (!forceRefresh) {
                    val cachedBoutiqueResponses =
                        boutiqueDatabase.boutiqueResponseDao().getBoutiqueResponses()
                    cachedBoutiqueResponses?.let {
                        if (it.isNotEmpty()) {
                            boutiqueResponses.postValue(it as ArrayList<BoutiqueResponse>)
                            return@withContext true
                        }
                    }
                }
                val response = boutiqueService.getBoutiqueResponses(businessId).execute()
                if (response.isSuccessful) {
                    response.body()?.let {
                        boutiqueDatabase.boutiqueResponseDao().apply {
                            truncateBoutiqueResponse()
                            insertBoutiqueResponse(it)
                            boutiqueResponses.postValue(it)
                            return@withContext true
                        }
                    }
                }
