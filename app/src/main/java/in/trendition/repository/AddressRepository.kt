...
class AddressRepository @Inject constructor(
    ...
    fun getOrderAddressMutableLiveData(): MutableLiveData<ArrayList<OrderAddress>> =
        orderAddressList
    ...
    suspend fun updateOrderAddressList(userId: Int, forceRefresh: Boolean): Boolean =
        withContext(Dispatchers.IO) {
            try {
                if (!forceRefresh) {
                    boutiqueDatabase.addressDao().getOrderAddressList()?.let {
    ...
    suspend fun verifyAndCapturePayment(
        ...
        try {
            ...
            return@withContext response.code() == 200 || response.code() == 204
        } catch (e: Exception) {
            e.printStackTrace()
        }
        return@withContext false
